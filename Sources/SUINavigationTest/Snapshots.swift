...
public let snapshotsDirName = "__Snapshots__"

public func rootNode(for view: any View, analyser: NavigationNodeAnalyserProtocol, mock: NavigationMockStore? = nil, isRecursive: Bool = false) -> NavigationNode {
    return analyser.searchNodes(for: view, isRecursive: isRecursive)
}

public func rootNode(for view: any View, mock: NavigationMockStore? = nil, isRecursive: Bool = false) -> NavigationNode {
...
public func assertSnapshot<V: View>(
    ...
    let snapshotDirectoryUrl = snapshotDirectory.map { URL(fileURLWithPath: $0, isDirectory: true) }
        ...
        .appendingPathComponent(snapshotsDirName)
